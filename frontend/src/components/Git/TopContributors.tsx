...
export const TopContributors: React.FC<TopContributorsProps> = ({ contributors }) => {
  return (
    <div className="bg-neutral-900 py-6 rounded-xl shadow-lg w-full h-full   max-w-3xl border border-neutral-700 text-white">
        <div className='text-2xl px-6 font-semibold mb-4 text-nowrap text-purple-400 '>Top Contributors</div>
        
      <ul className="space-y-2">
        {contributors.map((contributor, index) => (
          <li key={index} className="flex px-6 items-center space-x-4 overflow-auto h-full">
            <div>
