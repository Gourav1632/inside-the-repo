...
function Commits() {
  ...
  return (
    <div className="h-full w-full flex justify-center items-center relative overflow-y-scroll scroll-smooth">
      <GridBackground />
      ...
      <AnimatePresence mode="wait">
        {loading ? (
          ...
          <motion.div
            ...
            className="flex flex-col h-screen justify-center w-[60%] gap-4 items-center z-10"
          >
            <motion.div
              className="h-[16rem] flex justify-center gap-4 w-full items-center"
              initial="hidden"
              animate="visible"
              variants={{
            ...
            >
              {analysis && (
                <motion.div className="z-20 flex-2 h-full" variants={{ hidden: { opacity: 0, y: 30 }, visible: { opacity: 1, y: 0 } }}>
                  <ProjectSummary
                    repo={analysis.repo}
              ...
              {analysis?.top_contributors?.length > 0 && (
                <motion.div className="z-20 h-full flex-1" variants={{ hidden: { opacity: 0, y: 30 }, visible: { opacity: 1, y: 0 } }}>
                  <TopContributors contributors={analysis.top_contributors} />
                </motion.div>
              )}

              {analysis?.most_changed_files?.length > 0 && (
                <motion.div className="z-20 h-full flex-2" variants={{ hidden: { opacity: 0, y: 30 }, visible: { opacity: 1, y: 0 } }}>
